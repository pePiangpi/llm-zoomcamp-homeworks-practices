...
* **Custom SQLite Exporter:** Built a local SQLite exporter from the ground up to intercept, store, and query trace data locally [`traces.db`](./llm-zoomcamp-hw5/traces.db) without requiring heavy, external monitoring backends.
...
* **[`traces.db`](./llm-zoomcamp-hw5/traces.db)** — The local SQLite database storing captured execution traces, span names, timing metrics, and input token counts.
* **[`starter.py`](./llm-zoomcamp-hw5/starter.py) / Application Scripts** — Python scripts executing the RAG flow, handling OpenTelemetry setup, and recording data to the local exporter.
* **[`q6_check_tokens.py`](./llm-zoomcamp-hw5/q6_check_tokens.py)** — Data analysis script utilizing Pandas to verify token stability across multiple automated runs.
